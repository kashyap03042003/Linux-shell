...
#include <unistd.h>	  // system calls like chdir()....
#include <sys/types.h>
#include <signal.h> // signal() like ctrl+x and ctrl+z 
#include <fcntl.h>	// close(), open() file operations
int STATUSCODE = 1;
//Templates and MACROS
#define MAXCOMM 4   // assuming max number of commands to be 4
#define fori(a,b) for(int i=a;i<b;i++)
#define forie(a,b) for(int i=a;i<=b;i++)
#define scp(a,b) strcmp(a,b)
#define evp(a,b) execvp(a,b)
#define len(a) strlen(a)
#define MAXSTR 1024  // assuming maxlength of the string to be 1024
...
//************************************************************************************************************************
//responsible for forming a pipeline of commands
int parsePipeline(char *str, char **commands) {
    int i;
    for (i = 0; i < 2; i++) {
...
int parseInput(char* str, char** commands[])
{
	...
	if (parallel_Checker==1)
	{
		...
		return 0;    // returns 0 indicating parallel checker
	}
	...
	if (sequential_Checker==1)
	{
		...
		return 1;                //returns 1 indicating sequential checker
	}
	...
	if (redirect_Checker==1)
	{
		...
		return 2;                   //returns 2 indicating redirect checker
		
	}
	
	int pipeline_Checker=0;
    pipeline_Checker = parsePipeline(str, parser);
    if (pipeline_Checker==1) {
        parseSpace(parser[0], commands[0]);
        parseSpace(parser[1], commands[1]);
        return 6;                 // if this returns 6 then it indicates a pipeline command
    }
    
	parseSpace(str, commands[0]); //for single command cases

	if (scp("exit",commands[0][0])==0) //for exit case: here scp is strcmp which is declared as a macro for shortening the writing
	{	 
        return 3;
    }
...
void executeSequentialCommands(char** cmd_args[])  // helps in running multiple commands
{
	...
	fori(0,size)
	{
		...
		executeCommand(cmd_args[i]);   //if executeCommand() function gives value of statuscode as -1 then come out of loop example if we encounter an exit command then STATUSCODE=-1 and it comes out of loop
		if (STATUSCODE == -1){
			flag=1;
			break;
        }
...
void executeCommandRedirection(char** cmd_args[])   //output redirected to user specified output file
{
	...
	{
		pid_t pid = fork();   //forking a child process
		if(pid!=0) //necessarily a parent process or we can say pid>0 as well
		{
		    wait(NULL); //wait for child process to complete
		    return;
		}
		else if (pid == 0)       //child process is executed
		{
...
//*************************************************************************************************************
void executePipelineCommand(char **commands[]) {
    int pipefd[2];
    pid_t pid1, pid2;
    ...
    if (pid1 == 0) {
        ...
        if (evp(commands[0][0], commands[0]) < 0) {
        ...
        if (pid2 == 0) {
            ...
            close(pipefd[0]); // Close the read end of the pipe
            if (evp(commands[1][0], commands[1]) < 0) {
...
int main()
{
	...
	while (flag==1) 
	{
		...
		else if (parseStatus == 1)
		{
			...
			if (STATUSCODE == -1)  //status code becomes -1 if above function exits
			{
				break;
			}
		}
		...
		else if (parseStatus == 4) //comes to this case when user gives cd command.
		{
            ...
            executePipelineCommand(command_args);       //excuting pipeline commands
        }
		else
		{
